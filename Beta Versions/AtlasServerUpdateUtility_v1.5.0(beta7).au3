...
#AutoIt3Wrapper_Outfile=Builds\AtlasServerUpdateUtility_v1.5.0(beta7).exe
...
$aUtilVerBeta = "v1.5.0(beta7)" ; (2019-03-26)
...
Func ImportConfig($tServerDirLocal, $tConfigFile)
	...
	Else
		...
		$xServerNames = _StringBetween($sConfigReadServer, """" & $kServerNames & """: """, """,")
...
;~ 	$aServerPID = ResizeArray($aServerPID)
	Global $aServerMem = _GetMemArrayRawAvg($aServerPID)
	$aGUIMainActive = True
...
;~ 	$CPUPercent = GUICtrlCreateLabel("%", 322, 27, 24, 17)
	...
	$aServerMem = _GetMemArrayRawAvg($aServerPID)
	For $i = 0 To ($aServerGridTotal - 1)
