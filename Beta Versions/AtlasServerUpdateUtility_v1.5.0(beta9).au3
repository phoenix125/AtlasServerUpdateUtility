...
#AutoIt3Wrapper_Outfile=Builds\AtlasServerUpdateUtility_v1.5.0(beta9).exe
...
$aUtilVerBeta = "v1.5.0(beta9)" ; (2019-03-26)
...
SplashTextOn($aUtilName, $aUtilName & " " & $aUtilVersion & " started." & @CRLF & @CRLF & "Importing settings from GridStartSelect.ini", 400, 110, -1, -1, $DLG_MOVEABLE, "")
...
EndIf
$aFirstModBoot = False
SplashTextOn($aUtilName, $aUtilName & " " & $aUtilVersion & " started." & @CRLF & @CRLF & "Preparing server start files.", 400, 110, -1, -1, $DLG_MOVEABLE, "")
...
SplashTextOn($aUtilName, $aUtilName & " " & $aUtilVersion & " started." & @CRLF & @CRLF & "Starting servers.", 400, 110, -1, -1, $DLG_MOVEABLE, "")
...
#Region ;**** Tray Menu ****
SplashTextOn($aUtilName, $aUtilName & " " & $aUtilVersion & " started." & @CRLF & @CRLF & "Preparing GUI. Getting server information.", 400, 110, -1, -1, $DLG_MOVEABLE, "")
...
Func F_ShowServerInfo()
	Run("notepad.exe """ & $aServerSummaryFile & """")
EndFunc   ;==>F_ShowServerInfo
Func F_ShowConfig()
	Run("notepad.exe " & $aIniFile)
EndFunc   ;==>F_ShowConfig
Func F_ShowLogFile()
	Run("notepad.exe " & $aLogFile)
EndFunc   ;==>F_ShowLogFile
...
Func GridStartSelect($sGridFile, $sLogFile)
	...
	Next
	For $i = 0 To ($aServerGridTotal - 1)
		$xStartGrid[$i] = IniRead($sGridFile, " --------------- RUN THE FOLLOWING GRID SERVER(S) (yes/no) --------------- ", "Start Server (" & $xServergridx[$i] & "," & $xServergridy[$i] & ") (yes/no)", $iniCheck)
	...
	Next
	For $i = 0 To ($aServerGridTotal - 1)
		$xLocalGrid[$i] = IniRead($sGridFile, " --------------- LOCAL GRID SERVER(S) (yes-Local, no-Remote) (yes/no) --------------- ", "Is Server (" & $xServergridx[$i] & "," & $xServergridy[$i] & ") Local (yes/no)", $iniCheck)
	...
	Next
	If $iIniFail > 0 Then
		GridFileStartCheck($sGridFile, $iIniFail)
	EndIf
EndFunc   ;==>GridStartSelect
Func GridFileStartCheck($sGridFile, $iIniFail)
...
Func UpdateGridSelectINI($sGridFile)
	;	FileWriteLine($sIniFile, "[ --------------- " & StringUpper($aUtilName) & " INFORMATION --------------- ]")
	For $i = 0 To ($aServerGridTotal - 1)
		IniWrite($sGridFile, " --------------- RUN THE FOLLOWING GRID SERVER(S) (yes/no) --------------- ", "Start Server (" & $xServergridx[$i] & "," & $xServergridy[$i] & ") (yes/no)", $xStartGrid[$i])
	Next
	FileWriteLine($sGridFile, @CRLF)
	For $i = 0 To ($aServerGridTotal - 1)
		IniWrite($sGridFile, " --------------- LOCAL GRID SERVER(S) (yes-Local, no-Remote) (yes/no) --------------- ", "Is Server (" & $xServergridx[$i] & "," & $xServergridy[$i] & ") Local (yes/no)", $xLocalGrid[$i])
	Next
...
Func LogWrite($msg, $msgdebug = "blank")
	...
	If $msg <> "" Then
		...
		$aGUILogWindowText = _NowTime(5) & $msg & @CRLF & StringRight($aGUILogWindowText, 10000)
		If $aGUIReady Then GUICtrlSetData($LogWindow, $aGUILogWindowText)
	EndIf
	If $msgdebug <> "no" Then
...
;~ 	If $aFirstModCheck Then
;~ 		SplashTextOn($aUtilName, $aUtilName & " " & $aUtilVersion & " started." & @CRLF & @CRLF & "Checking for mod " & $sMod & @CRLF & $aModName[$i] & " update or new mod.", 400, 110, -1, -1, $DLG_MOVEABLE, "")
;~ 	EndIf
...
Func F_ExitCloseN()
	LogWrite(" [" & $aServerName & "] Utility exit without server shutdown initiated by user via tray icon (Exit: Do NOT Shut Down Servers).")
...
Func F_RestartNow()
	LogWrite(" [Server] Restart Server Now requested by user via tray icon (Restart Server Now) Redis will remain running.")
...
Func F_SendMessage($tAllorSel = "ask")
	LogWrite(" [Remote RCON] Broadcast message requested by user via tray icon (Send message).")
	SplashOff()
	;	$tMsg = ""
...
Func F_SendRCON($tAllorSel = "ask")
	LogWrite(" [Remote RCON] Send RCON command requested by user via tray icon (Send command).")
	SplashOff()
	;	$tMsg = ""
	If $aGridSomeDisable And $tAllorSel = "ask" Then
...
;~ 			For $i = 0 To UBound($tReturn - 1)
		...
		EndIf
	EndIf
;~ 	$aServerPID = ResizeArray($aServerPID, $aServersMax) ;Kim!!!
	Return $tReturn
...
Func GetPlayerCount($tSplash)
	...
	TraySetIcon($aIconFile, 201)
	GUICtrlSetData($LabelUtilReadyStatus, "Getting Players")
	;			If $tSplash Then
...
;~ 	$aServerPID = ResizeArray($aServerPID)
	Global $aServerPI_Stripped = ResizeArray($aServerPID, ($aServerGridTotal - 1))
	...
	Global $LogFile = GUICtrlCreateButton("Log File", 16, 160, 75, 25)
	...
	Global $LogWindow = GUICtrlCreateEdit("", 16, 608, 969, 73, BitOR($ES_AUTOVSCROLL, $ES_READONLY, $WS_VSCROLL))
	GUICtrlSetState($LogWindow, $GUI_FOCUS)
	DllCall('user32.dll', 'int', 'HideCaret', 'hwnd', '')
...
;~ 		_GUIImageList_AddIcon($hImage, @SystemDir & "\shell32.dll", 165)
	...
	For $i = 0 To ($aServerGridTotal - 1)
		...
		If (UBound($aServerPlayers) = $aServerGridTotal) And ($aServerPlayers[$i] <> "") Then
		...
		Else
			...
			Else
				...
				Else
					...
					If (UBound($aServerPlayers) = $aServerGridTotal) And ($aServerPlayers[$i] <> "") And ($aServerPlayers[$i] > -1) And ($aServerOnlinePlayerYN = "yes") Then
...
;~ 				_GUICtrlListView_AddSubItem($MainListViewWindow, $i, "", 2, 3)
		...
		Else
			...
			Else
				...
				Else ; OFFLINE
					...
					If (UBound($aServerPlayers) = $aServerGridTotal) And ($aServerPlayers[$i] <> "") And ($aServerPlayers[$i] > -1) And ($aServerOnlinePlayerYN = "yes") Then ; REMOTE PLAYERS ONLINE
						_GUIListViewEx_SetColour($aGUIListViewEX, $cSWRunning & ";", $i, 11)
					EndIf
				EndIf
			EndIf
	...
	Global $LabelUtilReadyStatus = GUICtrlCreateLabel("Idle", 144, 568, 106, 17)
	...
	Global $TotalPlayersEdit = GUICtrlCreateEdit("", 504, 566, 49, 17, BitOR($ES_CENTER, $ES_READONLY))
...
;~ 	$aServerMem = _GetMemArrayRawAvg($aServerPID)$aServerPI_Stripped
		...
		Else ; Server Not running
			...
			Else
				If $xLocalGrid[$i] = "yes" Then ; Local Server
					...
					EndIf
				Else ; Remote Server
					If (UBound($aServerPlayers) = $aServerGridTotal) And ($aServerPlayers[$i] <> "") And ($aServerPlayers[$i] > -1) And ($aServerOnlinePlayerYN = "yes") Then ; Remote Server with Online Players
						$tMainLVW[$i][11] = "Running" ; Status #008000
						If $tMainLVW[$i][11] <> $aMainLVW[$i][11] Then
							_GUIListViewEx_SetColour($aGUIListViewEX, $cSWRunning & ";", $i, 11)
						EndIf
