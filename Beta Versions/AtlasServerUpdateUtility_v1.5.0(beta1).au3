...
#AutoIt3Wrapper_Outfile=Builds\AtlasServerUpdateUtility_v1.5.0(beta1).exe
...
$aUtilVerBeta = "v1.5.0(beta1)" ; (2019-03-17)
...
EndIf
Global $aPlayerCountShow = True
TraySetState($TRAY_ICONSTATE_SHOW) ; Show the tray menu.

Global $aLineCount = 50 + $aServerGridTotal * 15 ;Create Show Online Players Window Frame
If $aLineCount > 800 Then $aLineCount = 800
Local $hGUI = GUICreate($aUtilName & " Online Players", 500, $aLineCount, -1, -1, BitOR($GUI_SS_DEFAULT_GUI, $WS_SIZEBOX, $WS_THICKFRAME)) ;Creates the GUI window
	...
	For $i = 0 To ($aServerGridTotal - 1)
		If ($xStartGrid[$i] = "yes") Then
			...
			Else
				...
				$tOnlinePlayers[1] = $tOnlinePlayers[1] & $xServergridx[$i] & $xServergridy[$i] & "(" & $tUserCnt & " " & $tUsers & ") "
				$tOnlinePlayers[2] = $tOnlinePlayers[2] & "Server " & $xServergridx[$i] & $xServergridy[$i] & ": " & $tUserCnt & " " & $tUsers & @CRLF
				$tOnlinePlayers[3] = $tOnlinePlayers[3] & "Server " & $xServergridx[$i] & $xServergridy[$i] & ": " & $tUserCnt & " " & $tUsers & @CRLF
			EndIf
			If $aRCONError Then
...
Func TrayShowPlayerCount()
	If $aPlayerCountShow = False Then
		...
		Global $aLineCount = 50 + $aServerGridTotal * 13 ;Create Show Online Players Window Frame
		If $aLineCount > 800 Then $aLineCount = 800
		Local $hGUI = GUICreate($aUtilName & " Online Players", 500, $aLineCount, -1, -1, BitOR($GUI_SS_DEFAULT_GUI, $WS_SIZEBOX, $WS_THICKFRAME)) ;Creates the GUI window
		GUICtrlSetResizing(-1, $GUI_DOCKAUTO)
		GUICtrlSetLimit(-1, 0xFFFFFF)
...
Func ShowPlayerCount()
	If $aPlayerCountShow Then
		;		Local $aLineCount = 50 + $aServerGridTotal * 13
		;		If $aLineCount > 800 Then $aLineCount = 800
		;		Local $hGUI = GUICreate($aUtilName & " Online Players", 500, $aLineCount, -1, -1, BitOR($GUI_SS_DEFAULT_GUI,$WS_SIZEBOX,$WS_THICKFRAME)) ;Creates the GUI window
		Local $iEdit = GUICtrlCreateEdit(_DateTimeFormat(_NowCalc(), 0) & @CRLF & $tOnlinePlayers[2], 0, 0, 500, $aLineCount, BitOR($ES_AUTOVSCROLL, $ES_NOHIDESEL, $ES_WANTRETURN, $WS_VSCROLL, $WS_HSCROLL, $ES_READONLY), $WS_EX_STATICEDGE)
		;		GUICtrlSetResizing(-1, $GUI_DOCKAUTO)
		;		GUICtrlSetLimit(-1, 0xFFFFFF)
