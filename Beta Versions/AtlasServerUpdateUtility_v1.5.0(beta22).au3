...
#AutoIt3Wrapper_Outfile=Builds\AtlasServerUpdateUtility_v1.5.0(beta22)_32bit(x86).exe
#AutoIt3Wrapper_Outfile_x64=Builds\AtlasServerUpdateUtility_v1.5.0(beta22).exe
...
$aUtilVerBeta = "v1.5.0(beta22)" ; (2019-04-21)
Global $aUtilVerNumber = 1 ; 0 = v1.5.0(beta19/20), 1 = v1.5.0(beta21/22)
...
Global $aNoExistingPID = True ; Used to determine whether to announce serrvers online. If no server PID file exists, it will announce.
Global $aGUIW = 500
Global $aGUIH = 250
...
Global $MainWindow = -1, $gOnlinePlayerWindow = 99999, $GUIMainWindow = -1 ; Predeclare GUI variables with dummy values to prevent firing the Case statements
...
;~ 		Local $tFile = FileOpen($aIniFile)
;~ 		Local $tTxt = FileRead($tFile)
;~ 		StringRegExpReplace($tTxt, "Server AltSaveDirectoryName(s) (comma separated. Use same order as listed in ServerGrid.json. Leave blank for default 00,01,10, etc) ###"
;~ 		IniWrite($aIniFile, " --------------- GAME SERVER CONFIGURATION --------------- ", "Server AltSaveDirectoryName(s) (comma separated. Use same order as listed in ServerGrid.json. Leave blank for default 00,01,10, etc) ###", "(CHANGED - SEE END OF THIS SECTION)")
	...
	If @exitMethod <> 1 Then
		...
		Else
			...
			ElseIf $Shutdown = 7 Then ; NO
				LogWrite(" [" & $aServerName & "] Server Shutdown - Initiated by User when closing " & $aUtilityVer & " Script")
				CloseServer($aServerIP, $aTelnetPort, $aTelnetPass, False) ; Do NOT close redis
				If $aServerUseRedis = "yes" Then
					PIDSaveRedis($aServerPIDRedis, $aPIDRedisFile)
				EndIf
				MsgBox(4096, $aUtilityVer, $aMsg, 20)
...
Func CloseServer($ip, $port, $pass, $tCloseRedisTF = True)
	If $aFirstBoot Then
...
;~ 	EndIf
	Else
		...
		If $tVer[0] = $tUtil Then
			...
			LogWrite(" [UTIL] " & $tUtilName & " up to date. Version: " & $tVer[0] & ", [UTIL] " & $tUtilName & " up To date. Version : " & $tVer[0] & ", Notes : " & $tVer[1])
...
Func W3_B_InstallServer()
	$tSplash = SplashTextOn($aUtilName, "Downloading and installing " & @CRLF & "SteamCMD and other mcrcon.exe (if needed).", 475, 110, -1, -1, $DLG_MOVEABLE, "")
	FileExistsFunc()
...
Func ConfigEdit($tSplash = 0, $tWarn = False)
	...
	Else
		...
		GUICtrlSetBkColor(-1, $cLWBackground)
		GUICtrlSetData(-1, $iIniErrorCRLF)
