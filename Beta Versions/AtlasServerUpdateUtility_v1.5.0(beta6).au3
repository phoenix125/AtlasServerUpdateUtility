...
#AutoIt3Wrapper_Outfile=Builds\AtlasServerUpdateUtility_v1.5.0(beta6).exe
...
$aUtilVerBeta = "v1.5.0(beta6)" ; (2019-03-26)
...
Global $aGUIH = 70 + $aServerGridTotal * 15 ;Create Show Online Players Window Frame
	...
	If $aServerOnlinePlayerYN = "yes" Then
		If $aPlayerCountShowTF Then
			...
			If $aPlayerCountWindowTF = False Then
				$hGUI = GUICreate($aUtilName & " Online Players", $aGUIW, $aGUIH, -1, -1, $WS_SIZEBOX) ;Creates the GUI window
				;				GUICtrlSetResizing(-1, $GUI_DOCKAUTO)
				;				GUICtrlSetLimit(-1, 0xFFFFFF)
			...
			If $tOnlinePlayerReady Then
				...
				;				GUICtrlSetData($iEdit, "")
				$iEdit = GUICtrlCreateEdit(_DateTimeFormat(_NowCalc(), 0) & @CRLF & $tOnlinePlayers[2], 0, 0, _WinAPI_GetClientWidth($hGUI), _WinAPI_GetClientHeight($hGUI), BitOR($ES_AUTOHSCROLL, $ES_NOHIDESEL, $ES_WANTRETURN, $WS_HSCROLL, $WS_VSCROLL, $ES_READONLY)) ; Horizontal Scroll, NO wrap text
			Else
				...
				$iEdit = GUICtrlCreateEdit(_DateTimeFormat(_NowCalc(), 0) & @CRLF & "Waiting for first Online Player check.", 0, 0, _WinAPI_GetClientWidth($hGUI), _WinAPI_GetClientHeight($hGUI), BitOR($ES_AUTOHSCROLL, $ES_NOHIDESEL, $ES_WANTRETURN, $WS_HSCROLL, $WS_VSCROLL, $ES_READONLY)) ; Horizontal Scroll, NO wrap text
				;				$aGUIClear = True
			EndIf
			ControlClick($hGUI, "", $iEdit)
			GUISetState(@SW_SHOW) ;Shows the GUI window
