...
#AutoIt3Wrapper_Outfile=Builds\AtlasServerUpdateUtility_v1.5.0(beta20)_32bit(x86).exe
#AutoIt3Wrapper_Outfile_x64=Builds\AtlasServerUpdateUtility_v1.5.0(beta20).exe
...
$aUtilVerBeta = "v1.5.0(beta20)" ; (2019-04-19)
Global $aUtilVerNumber = 0 ; 0 = v1.5.0(beta19)
...
Global $cW1Background = "0x808080" ; Color LW Wizard Select Window Background
Global $cW2Background = "0x808080" ; Color LW Wizard Select Window Background
; **** Temp
...
#cs
	SplashOff()
	If $aRunWiz = 6 Then ; YES
	WizardSelect()
	If $aWizardSelect = 2 Then
	WizardExisting()
	ElseIf $aWizardSelect = 3 Then
	WizardNew()
	EndIf
...
;~ 	Global $aUsePuttytel = IniRead($sIniFile, " --------------- " & StringUpper($aUtilName) & " MISC OPTIONS --------------- ", "Use puttytel for telnet client? (yes/no) ###", $iniCheck)
	...
	ElseIf $aUpdateCheckInterval < 5 Then ;Kim!!!
...
Func UtilUpdate($tLink, $tDL, $tUtil, $tUtilName, $tSplash = 0, $tUpdate = "requested")
	Local $tUtilUpdateAvailableTF = False
	If $tUpdate = "yes" Or $tUpdate = "requested" Then
	...
	If $hFileOpen = -1 Then
		FileClose($hFileOpen)
		Local $hFileRead = _INetGetSource($tLink)
		If @error Then
...
;~ 				SplashTextOn($aUtilName, $aUtilName & " up to date . . .", 400, 110, -1, -1, $DLG_MOVEABLE, "")
	...
	Else
		...
		LogWrite(" [UTIL] !!! New " & $aUtilName & " update available. Installed version: " & $tUtil & ", Latest version: " & $tVer[0], " [UTIL] New " & $aUtilName & " update available. Installed version: " & $tUtil & ", Latest version: " & $tVer[0] & ", Notes: " & $tVer[1])
...
Func SelectServersStop()
	...
	TraySetToolTip("Stopping select server(s).")
	TraySetIcon($aIconFile, 201)
	GUICtrlSetImage($IconReady, $aIconFile, 203) ; Busy Icon
	GUICtrlSetData($LabelUtilReadyStatus, "Stop Server ")
	For $i = 0 To ($aServerGridTotal - 1)
		If _GUICtrlListView_GetItemChecked($MainListViewWindow, $i) Then
			...
			LogWrite(" [Server] " & $tMsg1 & " IP:" & $xServerIP[$i] & " , Port:" & $xServerRCONPort[$i + 1])
			$xStartGrid[$i] = "no"
			$aGridSomeDisable = True
...
;~ 	$aServerPID = ResizeArray($aServerPID)
	...
	Global $ServerInfo = GUICtrlCreateButton("Setup Wizard", 16, 64, 75, 25)
...
;~ 	Opt("GUIResizeMode", $GUI_DOCKLEFT + $GUI_DOCKTOP)
	...
	Global $WizardWindowSelect = GUICreate("AtlasServerUpdateUtility Setup Wizard", 906, 555, -1, -1, BitOR($GUI_SS_DEFAULT_GUI, $WS_MAXIMIZEBOX, $WS_SIZEBOX, $WS_THICKFRAME, $WS_TABSTOP))
	...
	GUISetState(@SW_SHOW)
	#EndRegion ### END Koda GUI section ###
	If WinExists($GUIMainWindow) Then
	Else
...
Func GUI_W1_B_Existing()
	...
	$aExitGUIW1 = True
	$aWizardSelect = 2
	If WinExists($GUIMainWindow) Then WizardExisting()
EndFunc   ;==>GUI_W1_B_Existing
Func GUI_W1_B_NewInstall()
	GUIDelete($WizardWindowSelect)
	$aExitGUIW1 = True
	$aWizardSelect = 3
	If WinExists($GUIMainWindow) Then WizardNew()
...
;~ 	Global $WizardWindowExist = GUICreate("AtlasServerUpdateUtility Setup Wizard", 906, 555, -1, -1, BitOR($GUI_SS_DEFAULT_GUI, $WS_MAXIMIZEBOX, $WS_SIZEBOX, $WS_THICKFRAME, $WS_TABSTOP))
	...
	$Label3 = GUICtrlCreateLabel("Click below to import existing settings from the ServerGrid.json file.", 104, 336, 392, 20)
	GUICtrlSetFont(-1, 10, 400, 0, "MS Sans Serif")
	...
	Global $W2_T2_R_DefaultA1 = GUICtrlCreateRadio("Default Scheme: A1,A2,A3,B1,B2,B3  (Coming soon!)", 100, 201, 281, 17)
	...
	Global $W2_T2_R_Custom2 = GUICtrlCreateRadio("Custom Method 2: Enter folders one-at-a-time using a new popup window for each server.", 100, 337, 449, 17)
	GUICtrlSetOnEvent(-1, "GUI_W2_T2_R_Custom2")
	$Label6 = GUICtrlCreateLabel("Server AltSaveDirectoryName(s) (comma separated. Use same order as servers are listed in ServerGrid.json", 116, 257, 508, 17)
	...
	Global $W2_T2_I_AltSaveDIR = GUICtrlCreateInput("A1,A2,A3,B1,B2,B3,C1,C2,C3", 116, 281, 625, 21)
	...
	GUICtrlSetState($W2_T3_R_Import, $GUI_CHECKED)
	...
	Global $W2_T3_R_Method2 = GUICtrlCreateRadio("Custom Method 2: Enter RCON ports one-at-a-time using a new popup window for each server.", 100, 417, 449, 17)
	...
	Global $W2_T4_R_UseServerIP = GUICtrlCreateRadio("Use Server IP (Requires port forwarding with router loopback)", 100, 121, 393, 17)
	GUICtrlSetOnEvent(-1, "GUI_W2_T4_R_UseServerIP")
	GUICtrlSetState($W2_T4_R_UseServerIP, $GUI_CHECKED)
	Global $W2_T4_R_CustomIP = GUICtrlCreateRadio("Custom IP", 100, 153, 65, 17)
	GUICtrlSetOnEvent(-1, "GUI_W2_T4_R_CustomIP")
	Global $W2_T3_I_RCONIP = GUICtrlCreateInput("127.0.0.1", 172, 153, 145, 21)
	...
	GUICtrlSetFont(-1, 12, 400, 0, "MS Sans Serif")
	Global $Tab5 = GUICtrlCreateTabItem("5 Priority Settings")
...
;~ GUICtrlSetData(-1, "Edit1")
	...
	FileClose($tFileOpen)
	GUICtrlSetData(-1, $tTxt)
	$Label29 = GUICtrlCreateLabel("Changes can be made anytime either to the AtlasServerUpdateUtility.ini file with Notepad or directly within this utility.", 176, 72, 547, 17)
	...
	GUICtrlSetFont(-1, 12, 400, 0, "MS Sans Serif")
	Global $W2_T7_B_Restart = GUICtrlCreateButton("RESTART", 402, 272, 75, 25)
	GUICtrlSetOnEvent(-1, "GUI_W2_T7_B_Restart")
	GUICtrlCreateTabItem("")
	GUISetState(@SW_SHOW)
	#EndRegion ### END Koda GUI section ###
	If WinExists($GUIMainWindow) Then
...
;~ Func GUI_W2_T1_B_SelectFolder()
;~ 	Local $tCtrlID = $W2_T1_I_AtlasDIR
;~ 	Local $tInput = FileSelectFolder("Please select " & $aUtilName & " installation folder", $aSourceFile, "XML File (*.xml)", 3, "prefabs.xml")
;~ 	If @error Then
;~ 		Local $tRead = GUICtrlRead($tCtrlID)
;~ 		GUICtrlSetData($tCtrlID, $tRead)
;~ 	Else
...
;~ 			Case $WizardWindowExist
;~ 			Case $WizardTabWindow
;~ 			Case $W2_T1_B_ImportSettings
;~ 			Case $W2_T1_B_SelectFolder
;~ 			Case $W2_T1_I_AtlasDIR
;~ 			Case $W2_T2_R_Default00
;~ 			Case $W2_T2_R_DefaultA1
;~ 			Case $W2_T2_R_Custom1
;~ 			Case $W2_T2_R_Custom2
;~ 			Case $W2_T2_B_Folders
;~ 			Case $W2_T2_I_AltSaveDIR
;~ 			Case $W2_T4_R_Import
;~ 			Case $W2_T3_R_Method1
;~ 			Case $W2_T3_R_Method2
;~ 			Case $W2_T3_B_Ports
;~ 			Case $W2_T3_I_RCONPorts
;~ 			Case $W2_T4_R_UseServerIP
;~ 			Case $W2_T4_R_CustomIP
;~ 			Case $W2_T3_I_RCONIP
;~ 			Case $W2_T4_E_GridStart
;~ 			Case $W2_T4_B_Save
;~ 			Case $W2_T4_B_Reset
;~ 			Case $W2_T5_I_AdminPass
;~ 			Case $W2_T5_I_MaxPlayers
;~ 			Case $W2_T5_I_ReservedSlots
;~ 			Case $W2_T5_I_SteamCMDExtraCMD
;~ 			Case $W2_T5_I_UpdateInterval
;~ 			Case $W2_T6_B_Save
;~ 			Case $W2_T6_B_Reset
;~ 			Case $W2_T6_E_Config
;~ 			Case $W2_T7_B_Restart
;~ EndFunc   ;==>WizardExisting
...
#EndRegion ; **** CPU Usage Code Section **** by Ascend4nt.  Visit https://sites.google.com/site/ascend4ntscode/performancecounters-pdh
