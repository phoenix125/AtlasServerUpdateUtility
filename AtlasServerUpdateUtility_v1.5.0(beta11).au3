...
#AutoIt3Wrapper_Outfile=Builds\AtlasServerUpdateUtility_v1.5.0(beta11).exe
...
$aUtilVerBeta = "v1.5.0(beta11)" ; (2019-03-26)
...
If $aServerModYN = "yes" Then
	...
	If $aUtilReboot = "no" And ((_DateDiff('n', $aCFGLastUpdate, _NowCalc())) >= $aUpdateCheckInterval) Then ; $aUtilReboot = A planned reboot of the util... no need to check for updates.
		$aFirstModCheck = True
		CheckMod($aServerModList, $aSteamCMDDir, $aServerDirLocal, $aSplashStartUp)
	EndIf
;~ 	SplashOff()
Else
	$aServerModCMD = ""
...
Func UpdateCheck($tAsk, $tSplash = 0)
...
;~ 		SplashTextOn($aUtilName, $aUtilName & " " & $aUtilVersion & @CRLF & @CRLF & "Retrieving installed version buildid.", 400, 110, -1, -1, $DLG_MOVEABLE, "")
	...
	If ($aLatestVersion[0] And $aInstalledVersion[0]) Then
		...
		Else
			...
			Else
				If $aFirstBoot Then
					Local $tTxt = $aStartText & "Server is Out of Date!" & @CRLF & "Installed Version: " & $aInstalledVersion[1] & @CRLF & "Latest Version: " & $aLatestVersion[1] & @CRLF & "Updating server . . ."
					If $tSplash > 0 Then
						ControlSetText($tSplash, "", "Static1", $tTxt)
					Else
						SplashTextOn($aUtilName, $tTxt, 400, 110, -1, -1, $DLG_MOVEABLE, "")
...
Func CheckMod($sMods, $sSteamCmdDir, $sServerDir, $tSplash = 0)
	Local $xError = False
