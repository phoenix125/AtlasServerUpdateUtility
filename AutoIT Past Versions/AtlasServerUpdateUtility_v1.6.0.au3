...
#AutoIt3Wrapper_Outfile=Builds\AtlasServerUpdateUtility_v1.6.0.exe
#AutoIt3Wrapper_Outfile_x64=Builds\AtlasServerUpdateUtility_v1.6.0_64-bit(x64).exe
...
$aUtilVerStable = "v1.6.0" ; (2019-05-17)
$aUtilVerBeta = "v1.6.0" ; (2019-05-17)
Global $aUtilVerNumber = 7 ; New number assigned for each config file change. Used to write temp update script so that users are not forced to update config.
; 0 = v1.5.0(beta19/20)
...
If $tStartedServersTF Then
	Local $aMsg = "The following Server(s) need to be started:" & @CRLF & $tServersToStart & @CRLF & @CRLF & _
			"Click (YES) to continue to start servers (or wait 10 seconds)." & @CRLF & _
		...
		If $tMB1 = 6 Then                     ; YES
			...
			For $i = 0 To ($aServerGridTotal - 1)
				If $xStartGrid[$i] = "yes" Then
					...
					IniWrite($aGridSelectFile, $aGridIniTitle[0], "Start Server (" & _ServerNamingScheme($i, $aNamingScheme) & ") (yes/no)", "no")
				EndIf
			Next
		Else
...
Func GUI_Main_B_UpdateGame() ; Kim!!!
	F_UpdateServCheck()
...
Func UpdateMod($sMod, $sModName, $sSteamCmdDir, $sServerDir, $iReason, $sModNo)
	...
	_Splash(" Mod " & $sMod & " " & $sModName & @CRLF & " update released or new mod." & @CRLF & "Downloading and installing mod update.")
