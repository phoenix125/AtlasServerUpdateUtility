...
#AutoIt3Wrapper_Outfile=Builds\AtlasServerUpdateUtility_v1.2.1.exe
...
#AutoIt3Wrapper_Res_ProductVersion=v1.2.1
...
; All Servers
$aUtilVersion = "v1.2.1" ; (2019-02-21)
...
;	FileWriteLine($aLogFile, _NowCalc() & " 0:" & _ArrayToString($xServerAltSaveDir)) ;KIM
...
If $xServerAltSaveDir = "" Then
	...
	Next
	If $aServerRCONImport = "yes" Then
		$xServerRCONPort = ImportRCON($aServerDirLocal, $yServerAltSaveDir, $aServerGridTotal)
	EndIf
Else
	If ($aServerRCONImport = "yes") Then
		$xServerRCONPort = ImportRCON($aServerDirLocal, $xServerAltSaveDir, $aServerGridTotal)
	EndIf
EndIf
...
If $aServerGridTotal <> (UBound($xServerAltSaveDir) - 1) And ($xServerAltSaveDir <> "") And ($aServerWorldFriendlyName <> "TempXY") Then
	SplashOff()
	Local $aErrorMsg = "[CRITICAL ERROR!] The number of grids does not match the number of AltSaveDirectoryNames listed in " & $aUtilName & ".ini." & @CRLF & "Grid Total:" & $aServerGridTotal & ". Number of Server AltSaveDirectoryName entries:" & (UBound($xServerAltSaveDir) - 1) & @CRLF & "Example: Server AltSaveDirectoryName(s) (comma separated. Leave blank for default a00 a01 a10, etc) ###" & @CRLF & @CRLF & "Please correct the AltSaveDirectoryName entries in " & $aUtilName & ".ini file and restart " & $aUtilName & "."
	FileWriteLine($aLogFile, _NowCalc() & $aErrorMsg)
	MsgBox($MB_OK, $aUtilName, $aErrorMsg)
	Exit
EndIf
...
Else
	For $i = 0 To ($aServerGridTotal - 1)
		$xServerStart[$i] = """" & $aServerDirFull & "\" & $aServerEXE & """ Ocean?ServerX=" & $xServergridx[$i] & "?ServerY=" & $xServergridy[$i] & "?AltSaveDirectoryName=" & $xServerAltSaveDir[$i + 1] & "?ServerAdminPassword=" & $aServerAdminPass & "?MaxPlayers=" & $aServerMaxPlayers & "?ReservedPlayerSlots=" & $aServerReservedSlots & "?QueryPort=" & $xServerport[$i] & "?Port=" & $xServergameport[$i] & "?SeamlessIP=" & $xServerIP[$i] & $aServerMultiHomeFull & $xTelnetCMD[$i] & " " & $aServerExtraCMD
		FileWriteLine($aLogFile, _NowCalc() & " Imported from ServerGrid.json: Server " & $xServergridx[$i] & "," & $xServergridy[$i] & " Port:" & $xServergameport[$i] & " GamePort:" & $xServerport[$i] & " SeamlessIP:" & $xServerIP[$i] & " RCONPort:" & $xServerRCONPort[$i + 1] & " Folder:" & $xServerAltSaveDir[$i + 1])
	Next
EndIf
;If $aServerRCONImport = "yes" Then
...
Func ReadUini($sIniFile, $sLogFile)
	...
	Global $aServerAltSaveDir = IniRead($sIniFile, " --------------- GAME SERVER CONFIGURATION --------------- ", "Server AltSaveDirectoryName(s) (comma separated. Leave blank for default a00,a01,a10, etc) ###", $iniCheck)
	...
	Else
		Global $xServerAltSaveDir = StringSplit($aServerAltSaveDir, ",")
	EndIf
...
Func UpdateIni($sIniFile)
	...
	IniWrite($sIniFile, " --------------- GAME SERVER CONFIGURATION --------------- ", "Server AltSaveDirectoryName(s) (comma separated. Leave blank for default a00,a01,a10, etc) ###", $aServerAltSaveDir)
...
Func ImportConfig($tServerDirLocal, $tConfigFile)
	...
	Else
		...
		$xServerIP = _StringBetween($sConfigRead, """" & $kServerip & """: """, """,")
		...
		$xServergridx = _StringBetween($sConfigRead, """" & $kServergridx & """: ", ",")
		;Global $aServergridx = _ArrayToString($xServergridx)
		$xServergridy = _StringBetween($sConfigRead, """" & $kServergridy & """: ", ",")
		...
		$xServergameport = _StringBetween($sConfigRead, """" & $kServergameport & """: ", ",")
...
Func ImportRCON($zServerDirLocal, $zServerAltSaveDir, $zServerGridTotal)
		...
		Else
			...
			Else
				...
				If $hRCON[$i] < 1 Then
					...
					FileWriteLine($aLogFile, _NowCalc() & $aErrorMsg)
					SplashOff()
					MsgBox($MB_OK, $aUtilName, $aErrorMsg & @CRLF & "Please add RCONEnabled=True and RCONPort=[port] to GameUserSettings.ini or add RCON ports to " & $aUtilName & ".ini and restart " & $aUtilName & ".")
					;$hRCON[0] = False
					Exit
				EndIf
				If $aDebug Then
					FileWriteLine($aLogFile, _NowCalc() & " RCON Port:" & $hRCON[$i])
				EndIf
			EndIf
		EndIf
		FileClose($hFileOpen)
...
Func UpdateCheck()
	...
	If $aUpdateSource = "1" Then
		If $aFirstBoot Then
			SplashTextOn($aUtilName, $aUtilName & " started." & @CRLF & @CRLF & "Downloading latest buildid from SteamDB.", 400, 100, -1, -1, $DLG_MOVEABLE, "")
		EndIf
		Local $aLatestVersion = GetLatestVerSteamDB($aSteamAppID, $aServerVer)
	Else
		If $aFirstBoot Then
			SplashTextOn($aUtilName, $aUtilName & " started." & @CRLF & @CRLF & "Downloading latest buildid from SteamCMD.", 400, 100, -1, -1, $DLG_MOVEABLE, "")
		EndIf
