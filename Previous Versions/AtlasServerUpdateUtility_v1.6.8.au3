...
#AutoIt3Wrapper_Outfile=Builds\AtlasServerUpdateUtility_v1.6.8.exe
#AutoIt3Wrapper_Outfile_x64=Builds\AtlasServerUpdateUtility_v1.6.8_64-bit(x64).exe
...
#AutoIt3Wrapper_Res_ProductVersion=v1.6.8
...
$aUtilVerStable = "v1.6.8" ; (2019-06-02)
$aUtilVerBeta = "v1.6.8" ; (2019-06-02)
Global $aUtilVerNumber = 11 ; New number assigned for each config file change. Used to write temp update script so that users are not forced to update config.
; 0 = v1.5.0(beta19/20)
...
;~ 	Global $aUsePuttytel = IniRead($sIniFile, " --------------- " & StringUpper($aUtilName) & " MISC OPTIONS --------------- ", "Use puttytel for telnet client? (yes/no) ###", $iniCheck)
	...
	If $iniCheck = $aBackupNumberToKeep Then
		...
		$iIniFail += 1
		$iIniError = $iIniError & "BackupNumberToKeep, "
	ElseIf $aBackupNumberToKeep < 1 Then
...
Func _ExitUtil($tSleepYN = True)
	If $aUseKeepAliveYN = "yes" Then IniWrite($aKeepAliveConfigFileFull, " --------------- ATLASSERVERUPDATEUTILITYKEEPALIVE --------------- ", _
...
;~ 	Local $tCMD = """" & $aFolderTemp & "7z a -r -tzip -ssw -x!ocean.*.atlas """ & $tFull & """ """ & $aServerDirLocal & "\ShooterGame\Saved\"" """ & $aServerDirLocal & "\ShooterGame\Server*.json"""""
	Local $tCMD = """" & $aFolderTemp & "7z"" a -r -tzip -ssw -x!ocean.*.atlas """ & $tFull & """ """ & $aServerDirLocal & "\ShooterGame\Saved\"" """ & $aServerDirLocal & "\ShooterGame\Server*.json"""
	LogWrite(" [Backup] Backup started. File:" & $tName, " [Backup] Backup initiated: " & $tCMD)
	Local $tPID = Run($tCMD, "")
;~ 	LogWrite("", " [Backup] Backup initiated: " & $tCMD)
	; 7z a -r -tzip -ssw -x!ocean.*.atlas "d:\Temp\Zip Archive\BackupDate02.zip" "D:\Game Servers\atlas dedicated server Working\ShooterGame\Saved\" "D:\Game Servers\atlas dedicated server Working\ShooterGame\Server*.json"
	Local $tTimer1 = TimerInit()
	Local $tExit = False
	...
	EndIf
	_Splash("Backup completed successfully." & @CRLF & @CRLF & $tFull, 3000)
EndFunc   ;==>_BackupGame
...
Func GetLatestModUpdateTime($sMod, $sShow)
	...
	If FileExists($sFilePath) Then
		...
		Else
		EndIf
;~ 		MsgBox(0,"Kim","File Exists. FRError:" & $aFDError)
	Else
;~ 		MsgBox(0,"Kim","File NOT Exist.")
	EndIf
...
;~ 	MsgBox(0, "Kim", "@Error:" & @error & @CRLF & "Error:" & $tErr & @CRLF & "sMod:" & $sMod & @CRLF & "tInet:" & $tInet & @CRLF & "sFilePath:" & $sFilePath)
	Sleep(100)
;~ 	InetClose($tInet)
	Local $hFileOpen = FileOpen($sFilePath, 0)
	Local $sFileRead = FileRead($hFileOpen)
...
;~ 	$tY += 32
	...
	Global $GUI_Main_B_BackupMenu = GUICtrlCreateButton("Backup Menu", 800, 56, 91, 25)
	GUICtrlSetResizing(-1, $GUI_DOCKRIGHT + $GUI_DOCKTOP + $GUI_DOCKSIZE)
	GUICtrlSetOnEvent(-1, "GUI_Main_B_BackupMenu")
	Global $GUI_Main_B_EventScheduler = GUICtrlCreateButton("Event Scheduler", 896, 56, 91, 25)
	GUICtrlSetResizing(-1, $GUI_DOCKRIGHT + $GUI_DOCKTOP + $GUI_DOCKSIZE)
