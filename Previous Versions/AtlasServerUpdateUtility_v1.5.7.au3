...
#AutoIt3Wrapper_Outfile=Builds\AtlasServerUpdateUtility_v1.5.7.exe
#AutoIt3Wrapper_Outfile_x64=Builds\AtlasServerUpdateUtility_v1.5.7_64-bit(x64).exe
...
#AutoIt3Wrapper_Res_ProductVersion=v1.5.7
...
$aUtilVerStable = "v1.5.7" ; (2019-05-12)
$aUtilVerBeta = "v1.5.7" ; (2019-05-12)
Global $aUtilVerNumber = 6 ; New number assigned for each config file change. Used to write temp update script so that users are not forced to update config.
; 0 = v1.5.0(beta19/20)
...
If $aRemoteRestartUse = "yes" Then
	...
	If $aRemoteRestartSocket = -1 Then ; Kim!!!
		SplashOff()
		MsgBox(0x0, "TCP Error", "Could not bind to [" & $aRemoteRestartIP & "] Check server IP, disable Remote Restart in INI, or check for multiple instances of this util using the same port." & @CRLF & @CRLF & _
...
Func SendDiscordMsg($sHookURLs, $sBotMessage, $sBotName = "", $sBotTTS = False, $sBotAvatar = "", $aServerPID = "0")
	...
	$tCMD = """" & $aDiscordSendWebhookEXE & """ """ & $sHookURLs & """ """ & $sBotMessage & """ """ & $sBotName & """"
	Local $mOut = Run($tCMD, @ScriptDir, @SW_HIDE, $STDERR_CHILD + $STDOUT_CHILD)
	$tErr = ProcessWaitClose($mOut, 1)
	If $tErr = 0 Then
		$aRCONError = True
	EndIf
	Local $tcrcatch = StdoutRead($mOut)
	LogWrite(" [Discord] Message sent: " & $sBotMessage, " [Discord] Message sent:""" & $sBotMessage & """. Message Response: " & ReplaceCRLF($tcrcatch)) ; Kim!!!
	#cs
	If (StringInStr($tcrcatch, "200") > 0) Or (StringInStr($tcrcatch, "204") > 0) Then
		LogWrite(" [Discord] Message sent: " & $sBotMessage, " [Discord] Message sent:""" & $sBotMessage & """. Message Response: " & ReplaceCRLF($tcrcatch))
	Else
	...
	;~ 	$oHTTPOST.Open("POST", StringStripWS($sHookURLs, 2) & "?wait=true", False) ; Original code
		$oHTTPOST.Open("POST", StringStripWS($sHookURLs, 3) & "?wait=true", False)
	;~ 		If @error Then LogWrite("[Discord] Object Error: Open, Error Code: " & @error ; Return SetError(1, 0, 0)
	;~ 	$oHTTPOST.Option($WinHttpRequestOption_SslErrorIgnoreFlags) = $WinHttpRequestOption_SslErrorIgnoreFlags_IgnoreAll     ;Kim!! Test code
		$oHTTPOST.Option(4) = 0x3300 ; ignore all SSL errors ; ; Kim!! Test code
	;~ 	$oHTTPOST.setOption(2, 0x3300)   ;<== Modified ; Kim!! Test code
	;~ 	$oHTTPOST.SetRequestHeader("Content-Type", "application/json")  ; Original code
		$oHTTPOST.SetRequestHeader("Content-Type", "multipart/form-data")
	;~ 		If @error Then LogWrite("[Discord] Object Error: SetRequestHeader, Error Code: " & @error
		$oHTTPOST.Send($sJsonMessage)
	;~ 		If @error Then LogWrite("[Discord] Object Error: Send, Error Code: " & @error
