...
#AutoIt3Wrapper_Outfile=Builds\AtlasServerUpdateUtility_v1.5.6.exe
#AutoIt3Wrapper_Outfile_x64=Builds\AtlasServerUpdateUtility_v1.5.6_64-bit(x64).exe
...
#AutoIt3Wrapper_Res_ProductVersion=v1.5.6
...
$aUtilVerStable = "v1.5.6" ; (2019-05-11)
$aUtilVerBeta = "v1.5.6" ; (2019-05-11)
...
;~ 	$oHTTPOST.SetRequestHeader("Content-Type", "application/json")  ; Original code
	...
	If @error Then LogWrite("[Discord] Object Error: SetRequestHeader, Error Code: " & @error  ; Return SetError(1, 0, 0)
	$oHTTPOST.Send($sJsonMessage)
	If @error Then LogWrite("[Discord] Object Error: Send, Error Code: " & @error  ; Return SetError(1, 0, 0)
	Local $oStatusCode = $oHTTPOST.Status
	Local $oReceived = $oHTTPOST.ResponseText
	LogWrite(" [Discord] Message sent: " & $sBotMessage, " [Discord] Message Status Code {" & $oStatusCode & "} " & "Message Response: " & $oReceived)
	$aObjErrFunc = $tObjErrFunc
EndFunc   ;==>SendDiscordMsg

Func Webhook($Message) ; Example Discord script
...
Func ShowGUITools()
	...
	GUICtrlSetFont(-1, 8, 800, 0, "MS Sans Serif")
	$T1_L_DuplicatePortChecker = GUICtrlCreateLabel("Check for duplicate ports assigned in the ServerGrid.json file and ROCN ports.", $tLabelX, $tLabelY, 542, $tLabelH)
	GUICtrlSetFont(-1, 10, 800, 0, "MS Sans Serif")
	GUICtrlCreateGroup("", -99, -99, 1, 1)
	$tButtonY += 48
	$tLabelY += 48
