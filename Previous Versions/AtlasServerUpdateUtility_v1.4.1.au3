...
; All Servers
$aUtilVersion = "v1.4.1" ; (2019-03-08)
...
$aFirstBoot = True
;SplashTextOn($aUtilName, $aUtilName & " started." & @CRLF & @CRLF & "Preparing initial update check.", 400, 110, -1, -1, $DLG_MOVEABLE, "")
...
Func Gamercide()
	...
	If @exitMethod <> 1 Then
		...
		EndIf
	Else
		;	MsgBox(4096, "Kim!!!", "Shutdown command", 20)
		;		FileWriteLine($aLogFile, _NowCalc() & " [" & $aServerName & "] Server Shutdown - Initiated by User when closing " & $aUtilityVer & " Script")
		;		CloseServer($aServerIP, $aTelnetPort, $aTelnetPass)
		;		SplashOff()
		;		If ProcessExists($aServerPIDRedis) And $aServerUseRedis = "yes" Then
...
Func CheckMod($sMods, $sSteamCmdDir, $sServerDir)
	...
	For $i = 1 To $aMods[0]
		...
		ElseIf Not $aInstalledTime[0] Then
			$bStopUpdate = UpdateMod($aMods[$i], $aModName[$i], $sSteamCmdDir, $sServerDir, 0) ;No Manifest. Download First Mod
			If $bStopUpdate Then ExitLoop
		ElseIf Not $aInstalledTime[1] Then
			$bStopUpdate = UpdateMod($aMods[$i], $aModName[$i], $sSteamCmdDir, $sServerDir, 1) ;Mod does not exists. Download
			If $bStopUpdate Then ExitLoop
		ElseIf $aInstalledTime[1] And (StringCompare($aLatestTime[2], $aInstalledTime[2]) <> 0) Then
			$xError = True
			$bStopUpdate = UpdateMod($aMods[$i], $aModName[$i], $sSteamCmdDir, $sServerDir, 2) ;Mod Out of Date. Update.
			If $bStopUpdate Then ExitLoop
		EndIf
	Next
	...
	If $xServerAltSaveDir = "" Then
		For $i = 0 To ($aServerGridTotal - 1)
			...
			FileWriteLine($tServerSummaryFile, "[Server " & $xServergridx[$i] & $xServergridy[$i] & "] Use:" & $xStartGrid[$i] & ", QueryPort:" & $xServerport[$i] & ", Port:" & $xServergameport[$i] & _
					", SeamlessIP:" & $xServerIP[$i] & ", RCON:" & $xServerRCONPort[$i + 1] & ", DIR:a" & $xServergridx[$i] & $xServergridy[$i])
		Next
	Else
		For $i = 0 To ($aServerGridTotal - 1)
			If $xStartGrid[$i] = "no" Then
			...
			EndIf
			FileWriteLine($tServerSummaryFile, " [Server " & $xServergridx[$i] & $xServergridy[$i] & "] Use:" & $xStartGrid[$i] & " QueryPort:" & $xServerport[$i] & ", Port:" & $xServergameport[$i] & _
					", SeamlessIP:" & $xServerIP[$i] & ", RCON:" & $xServerRCONPort[$i + 1] & ", DIR:" & $xServerAltSaveDir[$i])
		Next
	EndIf
	Local $aWAN = _GetIP()
...
Func PIDSaveRedis($tPID, $tFile)
	;	MsgBox(0,"Kim","RedisPID File:" & $tPID & " | File:" & $tFile)	;KIM!!!
...
Func PIDReadRedis($tFile)
	...
	If $tTmp = -1 Then
		$tReturn = "0"
		FileWriteLine($aLogFile, _NowCalc() & " Redis Server NOT running or file not found.") ;KIM!!!
	Else
...
Func PIDReadServer($tFile)
	...
	If $tTmp = -1 Then
		$tReturn[0] = "0"
		FileWriteLine($aLogFile, _NowCalc() & " Grid Server(s) NOT running or file not found.") ;KIM!!!
	Else
		...
		Sleep(2500)
		;		SplashOff()
		;		MsgBox(0,$aUtilName,"Running servers found." & @CRLF & "PID(s):" & $tString,2)
		;		_ArrayDisplay($tReturn) ;Kim!!!
		For $i = 0 To $n
			...
			EndIf
		Next
		;FileWriteLine($aLogFile, _NowCalc() & " Grid Server(s) already running. PID(" & $tReturn1 & ") Imported from:" & $tFile)
	EndIf
	Return $tReturn
...
;If FileExists($aPIDServerFile) Then
;	Local $aTmp = FileRead($aPIDServerFile)
;	Global $aServerPID = StringSplit($aTmp,"|")
;	FileWriteLine($aLogFile, _NowCalc() & " Servers already running. PID(" & _ArrayToString($aServerPID) & ") Imported from:" & $aPIDServerFile)
;Else
;EndIf
