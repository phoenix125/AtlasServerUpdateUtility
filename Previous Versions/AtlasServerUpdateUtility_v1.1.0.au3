...
#AutoIt3Wrapper_Outfile=Builds\AtlasServerUpdateUtility_v1.1.0.exe
...
; All Servers
$aUtilVersion = "v1.1.0" ; (2019-02-16)
...
If $xServerAltSaveDir = "" Then
	For $i = 0 To ($aServerGridTotal - 1)
		$xServerStart[$i] = """" & $aServerDirFull & "\" & $aServerEXE & """ Ocean?ServerX=" & $xServergridx[$i] & "?ServerY=" & $xServergridy[$i] & "?AltSaveDirectoryName=a" & $xServergridx[$i] & $xServergridy[$i] & "?ServerAdminPassword=" & $aServerAdminPass & "?MaxPlayers=" & $aServerMaxPlayers & "?ReservedPlayerSlots=" & $aServerReservedSlots & "?QueryPort=" & $xServergameport[$i] & "?Port=" & $xServerport[$i] & "?SeamlessIP=" & $xServerIP[$i] & $aServerMultiHomeFull & $xTelnetCMD[$i] & " " & $aServerExtraCMD & " """
		FileWriteLine($aLogFile, _NowCalc() & " Imported from ServerGrid.json: Server " & $xServergridx[$i] & "," & $xServergridy[$i] & " Port:" & $xServerport[$i] & " GamePort:" & $xServergameport[$i] & " SeamlessIP:" & $xServerIP[$i] & " RCONPort:" & $xServerRCONPort[$i + 1] & " Folder:a" & $xServergridx[$i] & $xServergridy[$i])
	Next
Else
	For $i = 0 To ($aServerGridTotal - 1)
		...
		FileWriteLine($aLogFile, _NowCalc() & " Imported from ServerGrid.json: Server " & $xServergridx[$i] & "," & $xServergridy[$i] & " Port:" & $xServerport[$i] & " GamePort:" & $xServergameport[$i] & " SeamlessIP:" & $xServerIP[$i] & " RCONPort:" & $xServerRCONPort[$i + 1] & " Folder:" & $xServerAltSaveDir[$i + 1])
	Next
EndIf
...
Func ReadUini($sIniFile, $sLogFile)
	...
	Global $aServerRCONPort = IniRead($sIniFile, " --------------- GAME SERVER CONFIGURATION --------------- ", "Server RCON Port(s) (comma separated, grid order left-to-right ex. 00 01 02 10 11 12) ###", $iniCheck)
	...
	If $iniCheck = $aServerRCONPort Then
		...
		$iIniFail += 1
		$iIniError = $iIniError & "aServerReservedRCONPort, "
	EndIf
	If $iniCheck = $aServerAltSaveDir Then
		$aServerAltSaveDir = ""
		$iIniFail += 1
		$iIniError = $iIniError & "aServerAltSaveDir, "
	EndIf
...
Func UpdateIni($sIniFile)
	...
	IniWrite($sIniFile, " --------------- GAME SERVER CONFIGURATION --------------- ", "Server RCON Port(s) (comma separated, grid order left-to-right ex. 00 01 02 10 11 12) ###", $aServerRCONPort)
...
Func CloseServer($ip, $port, $pass)
	...
	;	EndIf
	SplashTextOn($aUtilName & ": " & $aServerName, "Waiting 10 seconds for server(s) to finish saving world . . .", 550, 75, -1, -1, $DLG_MOVEABLE, "")
	Sleep(10000)
	SplashOff()
	...
	If ProcessExists($aServerPID[0]) Then
		SplashTextOn($aUtilName & ": " & $aServerName, "Server(s) still saving. . . " & @CRLF & "Waiting 30 more seconds for server(s) to finish saving world . . .", 550, 75, -1, -1, $DLG_MOVEABLE, "")
		Sleep(30000)
	EndIf
...
Func ImportConfig($tServerDirLocal, $tConfigFile)
	...
	If $sFileExists = 0 Then
		...
		Else
			...
			Global $aServerWorldFriendlyName = "Atlas Temp"
	...
	EndIf
	;MsgBox($MB_OK,"test", $aServerWorldFriendlyName & @CRLF & $aServergridx)
	;For $i = 0 to ($aServerGridTotal - 1)
	;MsgBox($MB_OK,"test", "X=" & $xServergridx[$i] & " , Y=" & $xServergridy[$i])
	;Next
