...
#AutoIt3Wrapper_Outfile=Builds\AtlasServerUpdateUtility_v1.4.3.exe
...
#AutoIt3Wrapper_Res_ProductVersion=v1.4.3
...
Local $tMsg1 = $tTime3
For $tTime2 = 1 To $tTime3[0]
$tMsg1[$tTime2] = StringReplace($tMsg0[$tTime2], "\x", $tMod & " " & $aModName[$tTime2])
Next
Return $tMsg1
EndIf
EndFunc
...
$xStartGrid[$i] = "no "
EndIf
FileWriteLine($tServerSummaryFile, "[Server " & $xServergridx[$i] & $xServergridy[$i] & "] Use:" & $xStartGrid[$i] & ", QueryPort:" & $xServerport[$i] & ", Port:" & $xServergameport[$i] & ", SeamlessIP:" & $xServerIP[$i] & ", RCON:" & $xServerRCONPort[$i + 1] & ", DIR:" & $xServergridx[$i] & $xServergridy[$i])
Next
Else
For $i = 0 To ($aServerGridTotal - 1)
If $xStartGrid[$i] = "no" Then
