...
#AutoIt3Wrapper_Outfile=Builds\AtlasServerUpdateUtility_v1.7.5.exe
#AutoIt3Wrapper_Outfile_x64=Builds\AtlasServerUpdateUtility_v1.7.5_64-bit(x64).exe
...
#AutoIt3Wrapper_Res_ProductVersion=v1.7.5
...
$aUtilVerStable = "v1.7.5" ; (2019-06-14)
$aUtilVerBeta = "v1.7.5" ; (2019-06-14)
Global $aUtilVerNumber = 15 ; New number assigned for each config file change. Used to write temp update script so that users are not forced to update config.
; 0 = v1.5.0(beta19/20)
...
;~ 			$aFailedString = " Retry " & $x1 & ":"
			GUICtrlSetData($LabelUtilReadyStatus, "Recheck " & $x1 & ": Players 1sec")
			Sleep(1000)
			For $i = 0 To ($aServerGridTotal - 1)
...
;~ 	Local $tButtons = 6, $tGroupH = (32 * $tButtons + 17)
	...
	GUICtrlSetTip(-1, "Broadcast In Game User-Definable Preset Message to All Grids")
	$tY += $tButtonH + $tButtonGapY
...
;~ 			$aMainLVW[$i][8] = (Int($aServerMem[$i] / (1024 ^ 2))) & " MB" ; Memory
	...
	GUICtrlSetResizing(-1, $GUI_DOCKLEFT + $GUI_DOCKBOTTOM + $GUI_DOCKSIZE)
	Global $gPollRemoteServersCB = GUICtrlCreateCheckbox("Poll Remote", 305, 564)
	GUICtrlSetResizing(-1, $GUI_DOCKLEFT + $GUI_DOCKBOTTOM + $GUI_DOCKSIZE)
