...
#AutoIt3Wrapper_Outfile=Builds\AtlasServerUpdateUtility_v1.3.3.exe
...
#AutoIt3Wrapper_Res_ProductVersion=v1.3.3
...
; All Servers
$aUtilVersion = "v1.3.3" ; (2019-02-26)
...
;KIM!!!! UtilUpdate($aServerUpdateLinkVer, $aServerUpdateLinkDL, $aUtilVer)
...
Func ImportConfig($tServerDirLocal, $tConfigFile)
	...
	Else
		...
		; Items with Quotes
		Global $aServerModList = _ArrayToString(_StringBetween($sConfigRead, """" & $kServerModList & """: """, ""","))
...
Func FileExistsFunc()
	...
	If ($aServerModYN = "yes") Then
		...
		If $sFileExists = 0 Then
			...
			InetGet("https://github.com/phoenix125/Atlas-Mod-Downloader/raw/master/AtlasModDownloader.exe", @ScriptDir & "\AtlasModDownloader.exe", 1)
			;			DirCreate(@ScriptDir) ; to extract to
			;			_ExtractZip(@ScriptDir & "\mcrcon.zip", "", "mcrcon.exe", @ScriptDir)
			;			FileDelete(@ScriptDir & "\mcrcon.zip")
...
Func UtilUpdate($tLink, $tDL, $tUtil)
	$tVer[3]
	...
	If $hFileOpen = -1 Then
		$aReturn[0] = False
	Else
		$tVer = StringSplit($hFileOpen, "^")
		If $tVer[1] = $tUtil Then
			Return = True
		Else
			FileWriteLine($aLogFile, _NowCalc() & " [UTIL] New " & $aUtilName & " update available. Go to: " & $tDL & " to download.")
			MsgBox($MB_YESNOCANCEL, $aUtilName, "New " & $aUtilName & " update available. " & @CRLF & "Installed version: " & $tUtil & @CRLF & "Latest version: " & $tVer[1] & "Update notes: " & $tVer[2] & @CRLF & @CRLF & "Click (YES) to download, but NOT install, to " & @ScriptDir & @CRLF & "Click (CANCEL) or wait 30 seconds for message to disappear.", 30)
		EndIf
	EndIf
