...
#AutoIt3Wrapper_Outfile=Builds\AtlasServerUpdateUtility_v1.3.0.exe
...
; All Servers
$aUtilVersion = "v1.3.0" ; (2019-02-24)
...
Func ImportConfig($tServerDirLocal, $tConfigFile)
	...
	If $aServerModList = "" Then
		If $aServerModYN = "yes" Then
			FileWriteLine($aLogFile, _NowCalc() & "NOTICE: ""Use this util to install mods and check for mod updates""=yes in " & $aUtilName & ".ini but no mods were listed in " & $aConfigFile & ".")
		EndIf
		$aServerModYN = "no"
	Else
...
Func FileExistsFunc()
	...
	If ($aServerModYN = "yes") Then
		...
		If $sFileExists = 0 Then
			...
			InetGet("https://github.com/phoenix125/Atlas-Mod-Downloader/raw/master/AtlasModDownloader.exe", @ScriptDir & "\AtlasModDownloader.exe", 0)
			;			DirCreate(@ScriptDir) ; to extract to
			;			_ExtractZip(@ScriptDir & "\mcrcon.zip", "", "mcrcon.exe", @ScriptDir)
			;			FileDelete(@ScriptDir & "\mcrcon.zip")
